...
async def api_latest():
    try:
        ...
        raise HTTPException(status_code=502, detail=f"최신 회차 조회 실패: {e}")
...
async def predict(req: PredictRequest, end_no: Optional[int] = None, n: int = 10):
    try:
        ...
        raise HTTPException(status_code=502, detail=f"예측 처리 실패: {e}")
...
async def api_range_freq(end_no: int, n: int = 10):
    try:
        ...
        raise HTTPException(status_code=502, detail=f"구간 빈도 계산 실패: {e}")
