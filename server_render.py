...
app = FastAPI(title="Lotto Predictor", version="3.4.0")
...
def _set_cache(n: int, payload: Dict[str, Any]):
    _round_cache[n] = payload; _round_ts[n] = time.time()
...
async def fetch_round_async(client: httpx.AsyncClient, n: int, timeout: float = 1.8) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        _set_cache(n, d); return d
    except Exception as e:
        log.warning(f"fetch_round_async({n}) failed: {e}"); return None
...
async def get_latest_round(lo: int = 1, hi: int = 1500) -> int:
    ...
    try:
        ...
        _latest_cache.update({"value": lo, "ts": now}); return lo
    except Exception as e:
...
def frequency(rows: List[Dict[str, Any]]) -> List[int]:
    f = [0]*46
    for r in rows:
        for n in r["nums"]: f[n]+=1
    return f
def freq_prob(freq: List[int]) -> List[float]:
    tot = sum(freq[1:])
    if tot == 0: return [0.0]*46
...
def pick_set_by_weights(wts: List[float], power: float = 1.0, seed: Optional[int]=None) -> List[int]:
    ...
    while len(s)<6 and trials<500:
        ...
        for i in range(1,46):
            acc+=w[i]
            if r<=acc:
                if i not in s: s.append(i); break
    s.sort(); return s
def explain_set(ns: List[int], freq: List[int]) -> List[Dict[str, Any]]:
    ...
    for n,v in zip(ns, vals):
        ...
        out.append({"n": n, "pct": pct, "basis": "출현빈도 기반", "freq": v})
    return out
    ...
    for i in range(k*3):
        if strategy=="보수형":
            ...
            for _ in range(rnd.randint(1,2)):
                ...
                if rep not in s: s[rnd.randrange(0,6)] = rep
            s = sorted(set(s))
            while len(s)<6:
                x = rnd.randint(1,45)
...
def fallback_demo() -> Dict[str, Any]:
    rnd = random.Random(42); rows=[]; base_round=1200
    for i in range(120):
...
async def api_analysis(last: int = Query(120, ge=20, le=2000)):
    try:
        ...
        freq_arr = base["freq"]
        tot = max(1, sum(freq_arr[1:]))
        prob = [0.0]+[max(1e-9, f)/tot for f in freq_arr[1:]]
        picks = []
        for i in range(5):
        ...
        return JSONResponse({
            "latest": base["latest"], "count": base["count"],
        ...
        demo = fallback_demo(); freq_arr = demo["freq"]
        tot = max(1, sum(freq_arr[1:]))
        prob = [0.0]+[max(1e-9, f)/tot for f in freq_arr[1:]]
        picks = []
        for i in range(5):
            ...
            picks.append({"numbers": s, "reasons": explain_set(s, freq_arr)})
        intervals = interval_freq(freq_arr); weekly = weekly_best_strategy(freq_arr, demo["recent10"])
        strat_detail = []
        for name in ["보수형","균형형","고위험형"]:
            combos = top_k_sets(freq_arr, name, k=5)
...
def _open_browser(port: int):
    try: webbrowser.open(f"http://127.0.0.1:{port}/", new=2)
...
if __name__ == "__main__":
    ...
    _open_browser(port)
    uvicorn.run("server_render:app", host="127.0.0.1", port=port, log_level="info")
