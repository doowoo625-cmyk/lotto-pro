# Full UI + Hardened FastAPI server
...
app = FastAPI(title="Lotto Predictor", version="1.3.2")
...
async def fetch_round_async(client: httpx.AsyncClient, n: int, timeout: float = 5.0):
    cached = _get_cache(n)
...
async def fetch_range(start: int, end: int, batch_size: int = 50):
    results = []
...
def features(rows: List[Dict[str, Any]]):
    out = []
    for r in rows:
        nums = r["nums"]
        s = sum(nums)
...
def frequency(rows: List[Dict[str, Any]]):
    f = [0]*46
    for r in rows:
        for n in r["nums"]:
            f[n]+=1
...
async def api_stats(last: int = Query(50, ge=1, le=2000)):
    try:
        ...
        payload = {"latest": latest, "count": len(rows), "freq": freq, "tiers": tiers, "recent10": feats[-10:][::-1]}
