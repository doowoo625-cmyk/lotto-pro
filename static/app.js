...
async function refreshStatus(){
  ...
  }catch(e){ document.getElementById("apiStatus").textContent = "오프라인"; }
  try{
...
function renderAll(all){
  const box = document.getElementById("allCandidates"); box.innerHTML = "";
  Object.keys(all).forEach(k=>{
    const group = document.createElement("div"); group.innerHTML = `<h3>${k}</h3>`;
    const grid = document.createElement("div"); grid.className = "cards";
    all[k].forEach(it=>{
      const card = document.createElement("div"); card.className = "card";
      card.innerHTML = `<div class="pills">${it.numbers.map(n=>`<span class="pill">${n}</span>`).join("")}</div>
                        <div>score: ${it.score}</div>
                        <div style="opacity:.85;font-size:12px">${it.rationale}</div>`;
      grid.appendChild(card);
    });
    group.appendChild(grid); box.appendChild(group);
  });
}
async function doPredict(){
  const seedVal = document.getElementById("seed").value;
  const countVal = parseInt(document.getElementById("count").value || "5", 10);
  const body = { count: countVal }; if (seedVal !== "") body.seed = parseInt(seedVal, 10);
  const res = await getJSON("/api/predict", { method:"POST", headers:{"Content-Type":"application/json"}, body: JSON.stringify(body) });
  renderPriority(res.priority_sorted); renderAll(res.all_candidates);
}
async function saveLast(){
  ...
  await getJSON("/api/last_draw", { method:"POST", headers:{"Content-Type":"application/json"}, body: JSON.stringify({draw_no, numbers, bonus}) });
  await refreshStatus(); alert("저장 완료");
}
